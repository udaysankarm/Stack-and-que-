...
{
	public:
	stackLL A,B; // our 2 stacks
	Node *front,*end; // ront and end
	queue()
	{
		front=NULL;
		end=NULL;
	}
 	void Enqueue(int Val) //adding values 
	{
		if(A.isEmpty())  // if it is the first element 
		{
			A.push(Val); // adding element to the stack
			front=A.top;//getting front
		}
		else
		{
			A.push(Val); // adding element to the stack
			end=A.top; //getting the end
		}
	}
	void Dequeue()
	{
		if(A.isEmpty()) // checking whether it is empty or not
		{
			cout<<"\n The deletion is not permitted since there are no remaing elements in the queue";
		}
		else
		{
				while(!A.isEmpty()) // while it is not empty
				{
					B.push(A.topD()); // we add the element at top to b
					A.pop();// delete the top elemrnt
				}
				B.pop();// deleting the last element
				while(!B.isEmpty())
				{
					A.push(B.topD());// pasting back the stack to A
					B.pop();
				}
		}
	}
