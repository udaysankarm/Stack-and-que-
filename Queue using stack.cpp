...
class Stack // our stack class
{
	...
	void PUSH(int VAL)//insert function
	{
		X.Enqueue(VAL);// insering
		top=X.end;//asinging top
	}
	void POP()//delet function
	{
		if(X.size()>0)// check whether empty or not
		{
			while(X.size()>1) // all elements except the last inputted element is transfered to another queue Y
			{
				Y.Enqueue(X.start->data);
				X.Dequeue();
			}
			X.Dequeue();// deleting last element
			while(Y.size()>0) // copy all elements back to X
			{
	...
	{
		X.display();// calling the display function of the queue
	}
	int TOP()
	{
		if(top==NULL)
		{
			cout<<"The stack is empty(the printed value will be zero but this is not an element of the stack) ";
			return 0;
		}
		else
		return top->data;
...
int  main()
{
	...
	cout<<P.TOP()<<endl;
	cout<<"\n####################\n";
	cout<<P.SIZE()<<endl<<P.IsEmpty()<<endl;
	P.POP();
	P.Display();
	P.PUSH(67);
	P.PUSH(6565);
